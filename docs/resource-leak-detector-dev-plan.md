...
- [ ] **9. Standalone attribution CLI.** Add a thin CLI entry point in the attribution module that accepts one or two raw-report file paths and emits the final leak report to stdout (or to a file). Distributed as an executable jar. Manual verification: run against a saved raw report from the integration tests and confirm output matches the inline run's output.
