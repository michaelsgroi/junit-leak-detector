...
- [ ] **8. Runtime fork-detection.** Implement the runtime fork-detection check described in the design doc. In `ResourceLeakMonitor.testPlanExecutionStarted`, write a per-fork marker file (containing the JVM PID and start timestamp) to `target/resource-leak-detector/forks/<pid>.marker`. Before writing, scan the directory for existing markers — any marker whose timestamp is within a recent window (e.g., 5 minutes) of "now" indicates a prior fork of the same `mvn test` invocation has already run, which means `forkCount > 1` or `reuseForks=false`. On detection, log at WARN naming the suspected misconfiguration, the implication (cross-class leaks invisible/under-attributed), and pointing to the orchestrator + a documented Surefire snippet. Do NOT refuse to run. Static `pom.xml` parsing is explicitly out of scope (per requirements). Add an integration-test variant that intentionally configures `forkCount=2, reuseForks=false` and asserts the WARN fires.
