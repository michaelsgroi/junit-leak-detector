...
- [ ] **10. Differential-ordering double-run mode (orchestrator module).** Implement the double-run orchestrator as a Maven plugin that invokes the test suite twice with full Surefire configuration controlled by the plugin (runOrder=alphabetical for run 1; runOrder=random + recorded seed for run 2; plus forkCount/reuseForks/autodetection flags). Each run produces its own raw report. After both runs complete, the plugin invokes the standalone attribution CLI with both reports; the attribution module intersects candidate sets and emits the final report. Add `runs=2` config switch. Include integration test that exercises the two-run path end-to-end.
